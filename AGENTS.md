...
- `notification-prefs.ts` — per-member category push preferences. **A row is an EXPLICIT CHOICE and the `muted` column is what it chose**; absence means "never touched it" and resolves to `CATEGORY_DEFS[category].defaultOn`, so nothing is ever seeded. Both directions write a row — un-muting is **not** a DELETE, because `reminders` and `presence` are `defaultOn: false` (opt-in) and for those the absence of a row is what "muted" means. **So you cannot answer "is this muted?" by testing for a row:** the suppressed members are mostly the ones with no row. Read through `getMutedCategories()` (resolved, per member+scope) or `getCategoryChoices()` + the pure `isPushMuted()` (bulk, one query per workspace). There is no `mutedUserIdsFor()` any more and a bare set of muted ids is the wrong shape to bring back. Also owns `presenceSilencedUserIds()`, the ONLY place the presence rule is written. Its live half is the member's own account-level choice and nothing else, and note the direction — the default is *silenced*, so it looks for the rows that opt **in**. `presence` is no longer `workspaceSwitchable`, so the every-workspace vote it still encodes can never reach a verdict. That vote is kept, inert, against the flag being flipped back — a workspace-level "off" would silence a member only when *every* active workspace they belong to has switched it off (never "any" — one workspace must not silence a member on behalf of another), and a member with no active workspace is not silenced by that half
...
- Render a notification category on a settings screen that does not carry that screen's flag. **The two screens partition the catalogue:** `/me/settings` filters on `memberMutable` (`reminders`, `presence`), `/ws/:slug/settings` on `workspaceSwitchable` (`approvals`, `announcements`). Neither shows the other's half, disabled or otherwise — a locked switch only invites someone to throw it and be told no. The server refuses either way (`setMuted()` and the member PATCH routes; `serialiseCategoriesOff()` / `parseCategoriesOff()`), so the filters are presentation and `CATEGORY_DEFS` is where the split is decided. Never give a category both flags
- Decide whether a member's push is muted by testing whether a `notification_prefs` row exists, seed such a row, or un-mute by deleting one. `reminders` and `presence` are opt-in (`defaultOn: false`), so absence means *suppressed* for them and *delivered* for the rest — always resolve through `CATEGORY_DEFS`
- Put the reminder time fields back on the admin Notifications tab without moving the `reminders` category with them — an admin-set schedule for a notification only the member can silence is the arrangement that was removed. `ReminderField` is kept unrendered in `NotificationsTab.tsx`, and that tab still loads and re-sends `checkin_reminder_at` / `checkout_reminder_at` on save so it cannot clear values it no longer shows
