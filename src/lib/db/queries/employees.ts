...
export async function getCelebrationsInMonth(
  ...
  /**
   * The occurrence of a month/day anniversary inside the viewed month, or null.
   *
   * Built through `Date.UTC` so 29 February in a non-leap year rolls to 1 March
   * exactly the way `nextOccurrence` already rolls it - and then falls out of
   * February's list, which is why the resulting month is re-checked rather than
   * assumed.
   */
  function occurrenceIn(originalIso: string): string | null {
    const d = new Date(Date.UTC(year, month - 1, Number(originalIso.slice(8, 10))))
    const iso = d.toISOString().slice(0, 10)
    return iso.slice(0, 7) === monthIso ? iso : null
  }
  ...
  for (const r of rows) {
    ...
    if (r.date_of_birth && Number(r.date_of_birth.slice(5, 7)) === month) {
      const iso = occurrenceIn(r.date_of_birth)
      if (iso) out.push({ employeeId: r.id, name, kind: 'birthday', occursOn: iso })
    }

    if (r.date_of_joining && Number(r.date_of_joining.slice(5, 7)) === month) {
      const iso = occurrenceIn(r.date_of_joining)
      // The joining date itself is not an anniversary - only the years after it.
      if (iso && year > Number(r.date_of_joining.slice(0, 4))) {
        out.push({
          employeeId: r.id,
          name,
          kind: 'anniversary',
